...
msg_result:  .asciiz "Wartosc wyrazenia to a="
msg_continue: .asciiz "\nCzy kontynuowac? (0 - nie, 1 - tak): "
expression_a: .asciiz "1. a = (c - b) / d"
expression_b: .asciiz "\n2. a = (c - d) * b"
expression_c: .asciiz "\n3. a = b * c - 2 * d"
msg_expression: .asciiz "\nZ ktorego wyrazenia policzyc: "
excepetion: .asciiz "Nie poprawny nr!\n"
...
.text    
loop:
    # wyswieltnie wszystkich wyrazen
    li $v0, 4
    la $a0, expression_a
    syscall
...
again:
    ...
    # gdyby nie poprawny number wyrazenia zostal podany 
    lw $t0, which
    beq $t0, 1, skip
    beq $t0, 2, skip
    beq $t0, 3, skip
    
    # wyswieltamy info z jest zle 
    li $v0, 4
    la $a0, excepetion
    syscall
    j again # skaczemy do poczatku
    skip:
...
calculate_1:
    ...
    mfhi $s0 # reszte z dzielenia to jest w rejestrze wysokim 
    sw $s0, mod # zapis do zminnej mod 
...
print_result:
    ...
    lw $t0, which # ladujemy to t0 zmienna which 
    beq $t0, 1, print_mod # gdy user wybral 1 wyrazenie to jeszzcze drukujemy reszte 
    j continue # else pomijamy i skaczemy do contiune 
print_mod:
    # pokazujemy info
    li $v0, 4
    la $a0, next_line
    syscall
    #drukujemy wynik modulo
    li $v0, 1
    lw $a0, mod 
    syscall
